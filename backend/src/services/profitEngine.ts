...
export interface SettlementFinancials {
  gross_sales: number;
  tiktok_fees: number;
...
export class ProfitEngine {
   ...
   */
  static async getProductCosts(skus: string[]): Promise<Map<string, ProductCosts>> {
    const costMap = new Map<string, ProductCosts>();
    if (skus.length === 0) return costMap;
    ...
    const rows = await query(sql, skus) as any[];
    rows.forEach((row) => {
      costMap.set(row.sku, {
        sku: row.sku,
   ...
   */
  static calculateCOGS(items: OrderItemCOGS[], costMap: Map<string, ProductCosts>) {
    let totalCogs = 0;
    let packaging = 0;
    let bubbleWrap = 0;
    let tape = 0;
   ...
   */
  static calculateProfit(financials: SettlementFinancials, cogs: number) {
    const {
      gross_sales,
      tiktok_fees,
