...
export class OCRService {
   ...
   */
  static async processWaybill(
    fileBuffer: Buffer,
    fileName: string,
    mimeType: string
  ): Promise<ExtractedWaybill> {
    let textContent = '';
   ...
   */
  private static async performImageOCR(imageBuffer: Buffer): Promise<string> {
    try {
   ...
   */
  private static parseText(text: string): Omit<ExtractedWaybill, 'isMatched' | 'matchedMethod'> {
   ...
   */
  private static async matchWithOrders(
    parsed: Omit<ExtractedWaybill, 'isMatched' | 'matchedMethod'>
  ): Promise<ExtractedWaybill> {
    ...
    if (orderId) {
      const rows = await query('SELECT order_id, tracking_number FROM orders WHERE order_id = ?', [orderId]) as any[];
      if (rows.length > 0) {
        isMatched = true;
        matchedMethod = 'ORDER_ID';
        if (!trackingNumber) trackingNumber = rows[0].tracking_number;
      }
    }

    // Search by Tracking Number if not matched
    if (!isMatched && trackingNumber) {
      const rows = await query('SELECT order_id, tracking_number FROM orders WHERE tracking_number = ?', [trackingNumber]) as any[];
      if (rows.length > 0) {
        isMatched = true;
        matchedMethod = 'TRACKING_NUMBER';
        if (!orderId) orderId = rows[0].order_id;
      }
    }
    ...
    if (isMatched && orderId) {
      ...
      if (items.length > 0) {
        productDetails = items.map(item => 
          `${item.product_name} (${item.sku || 'No SKU'}) - Variation: ${item.variation || 'None'} x${item.quantity}`
        ).join('\n');
      }
    }
