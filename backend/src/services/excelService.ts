...
export class ExcelService {
  ...
  private static async importOrders(headers: string[], rows: any[][], userId: number): Promise<ParseResult> {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          if (!isDuplicate) {
            // Save Order
            await conn.execute(
              `INSERT INTO orders (order_id, tracking_number, customer_name, phone_number, shipping_address, courier, order_status, total_amount, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
              [orderId, tracking, customer, phone, address, courier, status, price * quantity]
            );
            imported++;
          } else {
            duplicates++;
          ...
          const [existingItem] = await conn.execute(
            'SELECT id FROM order_items WHERE order_id = ? AND sku = ? AND variation = ?',
            [orderId, sku, variation]
          );
          if ((existingItem as any[]).length === 0) {
            await conn.execute(
              `INSERT INTO order_items (order_id, sku, product_name, variation, quantity, price)
               VALUES (?, ?, ?, ?, ?, ?)`,
              [orderId, sku || null, productName, variation, quantity, price]
            );
            ...
            if (sku && (status.toUpperCase() === 'COMPLETED' || status.toUpperCase() === 'SHIPPED' || status.toUpperCase() === 'DELIVERED')) {
              ...
              if ((product as any[]).length > 0) {
                ...
                // Log movement
                await conn.execute(
                  `INSERT INTO inventory_movements (sku, movement_type, quantity, notes, created_by)
                   VALUES (?, 'SALE', ?, ?, ?)`,
                  [sku, -quantity, `Deduction from Order Import: ${orderId}`, userId]
                );
              }
            }
          }
    ...
    // Audit Log
    await query(
      "INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'ORDER_IMPORT', 'ORDERS', ?)",
      [userId, `Imported ${imported} orders. Duplicates: ${duplicates}. Errors: ${errors.length}`]
    );
   ...
   */
  private static async importReturns(headers: string[], rows: any[][], userId: number): Promise<ParseResult> {
    const mapping = {
    ...
    await executeTransaction(async (conn) => {
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          // Insert return
          await conn.execute(
            `INSERT INTO returns (return_id, order_id, sku, tracking_number, reason, condition_status, return_shipping_cost, refunded_amount, returned_to_inventory, recorded_by)
             VALUES (?, ?, ?, ?, ?, 'GOOD', ?, ?, TRUE, ?)`,
            [returnId, orderId, sku || null, tracking, reason, returnShipping, refundAmount, userId]
          );
          ...
          if (sku) {
            ...
            if ((product as any[]).length > 0) {
              ...
              await conn.execute(
                `INSERT INTO inventory_movements (sku, movement_type, quantity, notes, created_by)
                 VALUES (?, 'RETURN', 1, ?, ?)`,
                [sku, `Restock from Return Import: ${returnId}`, userId]
              );
            }
          }
          ...
          const [settlements] = await conn.execute(
            'SELECT id, gross_sales, tiktok_fees, affiliate_commission, shipping_fee_subsidy, shipping_fee_actual, platform_discount, adjustments, tax FROM settlements WHERE order_id = ?',
            [orderId]
          );
          ...
          for (const sett of settlements as any[]) {
            ...
            await conn.execute(
              `UPDATE settlements 
               ...
               WHERE id = ?`,
              [refundAmount, returnShipping, recalculated.statement_amount, recalculated.net_profit, sett.id]
            );
          }
    ...
    await query(
      "INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'RETURN_IMPORT', 'RETURNS', ?)",
      [userId, `Imported ${imported} returns. Duplicates: ${duplicates}. Errors: ${errors.length}`]
    );
