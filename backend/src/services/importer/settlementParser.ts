...
import { ImportLogger, ImportLog } from './importLogger.js';
...
export class SettlementParser {
   ...
   * - Saves data to DB
   */
  static async parseAndImport(fileBuffer: Buffer, userId: number): Promise<{ success: boolean; log: ImportLog }> {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
          ...
          if (orderId) {
             ...
             const itemsList = (orderItems as any[]).map(item => ({ sku: item.sku || '', quantity: item.quantity }));
    ...
    await executeTransaction(async (conn) => {
        await conn.execute(
            ...
            [userId, `Imported ${logger.getLog().importedRows} settlements. Skipped: ${logger.getLog().skippedRows}. Warnings: ${logger.getLog().warnings.length}. Errors: ${logger.getLog().errors.length}`]
        );
    });
  ...
  private static parseAmount(row: any[], index: number, absolute = false): number {
