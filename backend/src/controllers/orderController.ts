...
export const getOrders = async (req: Request, res: Response) => {
  const {
    search, courier, status, startDate, endDate, supplier, sku, hasSettlement
  } = req.query;
  ...
  try {
    ...
    const ordersList = await query(sql, params) as any[];
    res.json(ordersList);
  } catch (err: any) {
...
export const getOrderDetails = async (req: Request, res: Response) => {
  ...
  try {
    // 1. Get Order Core
    const orders = await query('SELECT * FROM orders WHERE order_id = ?', [id]) as any[];
    if (orders.length === 0) {
    ...
    const items = await query(
      `SELECT oi.*, p.purchase_cost, p.packaging_cost, p.bubble_wrap_cost,
    ...
    // 3. Get Settlements
    const settlements = await query('SELECT * FROM settlements WHERE order_id = ?', [id]) as any[];

    // 4. Get Waybill
    const waybills = await query('SELECT * FROM waybills WHERE order_id = ?', [id]) as any[];

    // 5. Get Return History
    const returns = await query('SELECT * FROM returns WHERE order_id = ?', [id]) as any[];
