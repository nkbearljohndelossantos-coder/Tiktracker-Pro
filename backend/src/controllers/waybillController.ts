...
export const uploadWaybills = async (req: Request, res: Response) => {
  ...
  for (const file of files) {
    try {
      ...
      await executeTransaction(async (conn) => {
        ...
        const waybillId = (insertRes as any).insertId;
        ...
        if (!extracted.isMatched) {
          ...
          await conn.execute(
            `INSERT INTO waybill_review_queue (waybill_id, tracking_number, ocr_text, reason)
             VALUES (?, ?, ?, ?)`,
            [waybillId, extracted.trackingNumber || null, extracted.ocrText, reason]
          );
        }
      });
  ...
  // Audit log
  await query(
    "INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'WAYBILL_UPLOAD', 'WAYBILLS', ?)",
    [user.id, `Uploaded ${results.length} waybills. Failures: ${errors.length}`]
  );
  ...
  try {
    await executeTransaction(async (conn) => {
      ...
      const [queueRows] = await conn.execute(
        'SELECT waybill_id FROM waybill_review_queue WHERE id = ? AND is_resolved = FALSE',
        [queueId]
      );

      if ((queueRows as any[]).length === 0) {
        throw new Error('Queue item not found or already resolved.');
      }
      
      const waybillId = (queueRows as any[])[0].waybill_id;

      // Find order details
      const [orderRows] = await conn.execute(
        'SELECT order_id, tracking_number FROM orders WHERE order_id = ?',
        [orderId]
      );
      ...
      await conn.execute(
        `UPDATE waybills 
         ...
         WHERE id = ?`,
        [order.order_id, order.tracking_number || 'MANUAL-LINKED', waybillId]
      );

      // Update Queue status
      await conn.execute(
        `UPDATE waybill_review_queue 
    ...
    // Audit log
    await query(
      "INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'RESOLVE_WAYBILL', 'WAYBILLS', ?)",
      [user.id, `Manually resolved waybill queue ID: ${queueId} to Order: ${orderId}`]
    );
