...
import ProfitEngine, { SettlementFinancials } from '../services/profitEngine.js';
...
export const scanReturn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const orderRows = await query(
      'SELECT order_id, tracking_number FROM orders WHERE order_id = ? OR tracking_number = ?',
      [barcode, barcode]
    ) as any[];
    ...
    const items = await query(
      'SELECT sku, product_name, variation, quantity FROM order_items WHERE order_id = ?',
      [order.order_id]
    ) as any[];
 ...
 */
export const saveReturn = async (req: Request, res: Response) => {
  const {
    orderId, sku, returnId, reason, conditionStatus,
    returnShippingCost, refundedAmount, returnToStock
  } = req.body;
  ...
  try {
    await executeTransaction(async (conn) => {
      ...
      if (returnToStock && (conditionStatus === 'GOOD' || conditionStatus === 'DAMAGED_SELLABLE')) {
        await conn.execute(
          'UPDATE products SET inventory_qty = inventory_qty + 1 WHERE sku = ?',
          [sku]
        );
        ...
        await conn.execute(
          `INSERT INTO inventory_movements (sku, movement_type, quantity, notes, created_by)
           VALUES (?, 'RETURN', 1, ?, ?)`,
          [sku, `Restock from manual Return log: ${targetReturnId}`, user.id]
        );
      }
      ...
      for (const sett of settlements as any[]) {
        ...
        await conn.execute(
          `UPDATE settlements 
           ...
           WHERE id = ?`,
          [refundedAmount || 0, returnShippingCost || 0, recalculated.statement_amount, recalculated.net_profit, sett.id]
        );
      }
    });
    ...
    // Audit log
    await query(
      "INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'LOG_RETURN', 'RETURNS', ?)",
      [user.id, `Logged return ID: ${targetReturnId} for Order: ${orderId}`]
    );
