...
export const getProductBySku = async (req: Request, res: Response) => {
  ...
  try {
    const rows = await query('SELECT * FROM products WHERE sku = ?', [sku]) as any[];
    if (rows.length === 0) {
      ...
      if (inventory_qty > 0) {
        await conn.execute(
          `INSERT INTO inventory_movements (sku, movement_type, quantity, notes, created_by)
           VALUES (?, 'IMPORT', ?, ?, ?)`,
          [sku, inventory_qty, 'Initial stock entry upon product creation.', user.id]
        );
      }
    });
...
export const updateProduct = async (req: Request, res: Response) => {
  ...
  try {
    await executeTransaction(async (conn) => {
      ...
      const costChanged = 
        parseFloat(current.purchase_cost) !== parseFloat(purchase_cost || '0') ||
      ...
      if (stockDiff !== 0) {
        await conn.execute(
          ...
          [sku, stockDiff, 'Manual stock adjustment.', user.id]
        );
      }
    });
