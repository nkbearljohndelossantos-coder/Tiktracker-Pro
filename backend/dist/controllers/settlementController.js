...
export const importExcelReport = async (req, res) => {
    if (!req.file) {
...
export const recalculateSettlements = async (req, res) => {
    ...
    try {
        ...
        await executeTransaction(async (conn) => {
           ...
           WHERE id = ?`, [calculated.statement_amount, calculated.net_profit, sett.id]);
                updatedCount++;
            }
        });
        // Log action
        await query("INSERT INTO audit_logs (user_id, action, module, details) VALUES (?, 'RECALCULATE_FINANCES', 'FINANCE', ?)", [user.id, `Triggered bulk recalculation for ${updatedCount} orders.`]);
