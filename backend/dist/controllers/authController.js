...
const generateTokens = (user) => {
    const accessToken = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: JWT_EXPIRE });
...
export const register = async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO users (username, password_hash, email, role) VALUES (?, ?, ?, ?)', [username, passwordHash, email, role]);
...
export const refresh = async (req, res) => {
    ...
    try {
        ...
        const users = await query('SELECT id, username, role, email FROM users WHERE id = ?', [payload.id]);
        if (users.length === 0) {
